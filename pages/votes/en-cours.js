...
export async function getServerSideProps({req, res, query}) {
  ...
  const {data, total} = await voteController.list(limit, skip, filter)
...
const VoteEnCours = ({votes, user, pageNb, pages}) => {
  ...
  const handlePageChange = async (event, value) => {
    ...
    setPage(value);
    const res = await fetch(`/api${path}&inprogress=1`)
    const {data, total} = JSON.parse(await res.text())
    setVotesList(data)
    router.push(`/votes/en-cours?page=${value}`, undefined, { shallow: true })
  };
